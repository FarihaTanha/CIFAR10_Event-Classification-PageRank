...
This project is an experiment to see if computers can automatically figure out what kind of event is happening in a group of photos. I used an algorithm called PageRank to find the most important photos in a group. If the computer can identify the most important photos, it can guess the event much faster.

## The Problem

We take thousands of photos on our phones, but it is hard to organize them. The goal is to automatically label a group of photos (for example, identifying a "Trip to the beach") without looking at every single one.
